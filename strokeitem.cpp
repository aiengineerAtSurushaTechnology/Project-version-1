#include "strokeitem.h"
#include <QDebug>
#include <QGraphicsLineItem>
#include <QLineF>
#include <QPen>
StrokeItem::StrokeItem()
{

}

int StrokeItem::type() const
{
    return Type;
}
//Write
QDataStream &operator<<(QDataStream &out,const StrokeItem & mStroke)
{
    qreal posX= mStroke.scenePos().x();
    qreal posY = mStroke.scenePos().y();

    //Write position of item
    out << posX << posY;
    qDebug() <<"Write mStroke x and y "<<posX<<","<<posY;
    //Write line count
    int LC=mStroke.childItems().count();
    qDebug()<<"LC count: "<<LC;
    out<<LC;
    //out<<30;
    qDebug() <<"Write Linecount: "<< mStroke.childItems().count();
    //Write Pen
    foreach (QGraphicsItem * item, mStroke.childItems()) {
        QGraphicsLineItem * lineItem = dynamic_cast<QGraphicsLineItem*>(item);
        if(lineItem){
            out << lineItem->pen();
            break;
        }
    }

    //Write composing lines
    QList<QGraphicsItem*> children = mStroke.childItems();
    foreach (QGraphicsItem * item, children) {
        QGraphicsLineItem * lineItem = dynamic_cast<QGraphicsLineItem*>(item);
        if(lineItem){
            qDebug() <<"Writing out lines "<<lineItem->line();
            out << lineItem->line();
        }
    }

    return  out;
}
QDataStream &operator>>(QDataStream &in, StrokeItem & mStroke)
{
    qreal posX;
    qreal posY;

    //Read Pos
    in >> posX >> posY;
    qDebug() <<"Read mStroke x and y "<<posX<<","<<posY;
    //Read line count
    int lineCount ;
    in >> lineCount;
    qDebug() <<"Lines Count: "<<lineCount;
    //Read Pen
    QPen mPen;
    in >> mPen;

    QLineF mLine;

    for(int i=0; i < lineCount ; i++){
        in >> mLine;
        qDebug() <<"Read out lines "<<mLine;
        QGraphicsLineItem * lineItem = new QGraphicsLineItem(mLine);
        lineItem->setPen(mPen);
        mStroke.addToGroup(lineItem);

    }

    return in;
}
